...
int main()
{
    ...
    /*BigInt big1 = biFromString("1");
    for (int i = 0; i < 1024; i++) {
       biMul(big1, biFromInt(2));
    }
    
    BigInt big2 = biFromString("-1");
    for (int i = 0; i < 1024; i++) {
        biMul(big2, biFromInt(2));
    }
    biAdd(big2, biFromInt(1));
    biSub(big1)
    printf("%d\n", biCmp(big1, big2));
    char buf[10000];
    biToString(big1, buf, 10000);
    printf("%s\n", buf);
    biToString(big2, buf, 10000);
    printf("%s\n", buf);*/
