...
global biFromIntZban
global biCopy
global biFromStringZban
global biToStringZban
global biDeleteZban
global biSignZban
global biAddNew
global biAddZban
...
; BigInt biFromIntZban(int64_t x);
...
; BigInt biFromStringZban(char *a);
; a in rdi
; result in rax
biFromStringZban:
...
; void biToStringZban(BigInt a, char *s, size_t limit);
; a in rdi
; s in rsi
; limit in rdx
biToStringZban:
    ...
    pop r11
    mov rdi, r11
    call biDeleteZban ; free copied BigInt
    ret
    .notZero
    ...
    mov rdi, r11
    call biDeleteZban ; free BigInt copy
    ret


; void biDeleteZban(BigInt a);
; a in rdi
biDeleteZban:
    push rdi
    mov rdi, [rdi + 8]
...
; void biAddZban(BigInt a, BigInt b) { a += b }
; a in rdi
; b in rsi
biAddZban:
    push rdi
...
biSubNew:
    ...
    jne .areNotEqual2
    call biDeleteZban ; we need to delete it
    .areNotEqual2
    pop rax
...
; void biSubZban(BigInt a, BigInt b) { a -= b }
; a in rdi
; b in rsi
biSubZban:
    push rdi
...
; void biMulZban(BigInt a, BigInt b) { a *= b }
; a in rdi
; b in rsi
biMulZban:
    push rdi
...
biDivRem: ; it has not done yet
    ...
    jne .areNotEqual ; I will temporary modify num and denom, so their ptr should'n be equal
        ...
        push rsi
        mov rdi, 1
        call biFromIntZban ; quotient = 1
        push rax
        mov rdi, 0
        call biFromIntZban ; remainder = 0
        mov r9, rax
        pop rax
        mov r8, rax
        pop rsi
    ...
    mov rdi, 1
    call biFromIntZban ; toAdd
    mov r15, rax
    
    mov rdi, 2
    call biFromIntZban ; 2
    mov rbx, rax
    ...
    .while2
        ...
        mov rdi, r13
        mov rsi, r14
        call biCmpZban ; a >= b -> a -= b, ans += toAdd
        cmp eax, 0
        jl .aIsLess
            mov rdi, r13
            mov rsi, r14
    ...
    mov rdi, r14
    mov rsi, rbx
    call biMulZban ; b * quotient
    
    mov rdi, r13
    mov rsi, r14
    call biSubZban ; r13 now is remainder
...
; int biCmpZban(BigInt a, BigInt b);
; compares 2 BigInt's
; a in rdi
; b in rsi
; result in eax
biCmpZban:
    call biSubNew ; biCmpZban is equal to biSignZban(biSubNew(a - b))
    mov rdi, rax
    push rdi
    call biSignZban
    pop rdi
